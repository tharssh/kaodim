...
//  Kaodim
//
//  Created by InsightzClub on 21/02/2020.
...
class HomeVC: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
       super.viewWillAppear(true)
        Services.getServices(path: "home"){ (results) in
...
extension HomeVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if service[section].serviceTypes.count > 4 {
            return 4
        }else{
            return service[section].serviceTypes.count
        }
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if(indexPath.row == 3 && numberOfRows > 3){
            ...
            let sections = service[indexPath.section].serviceTypes
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if (indexPath.row == 3 && numberOfRows > 3){
            ...
            self.service = [service[indexPath.section]]
            self.performSegue(withIdentifier: "moreServices", sender: indexPath.row)
        }
    }
    
// MARK: - Navigation
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "moreServices"{
            let destinationVC = segue.destination as! MoreVC
            destinationVC.moreList = self.service
        }
    }
